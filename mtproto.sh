...
fix_mtp(){
  ...
  print_line
  echo -e "> Start to clear firewall rules/stop firewall/uninstall firewall..."
  print_line
...
if [[ "start" == $param ]]; then
  ...
  if [! -f "$WORKDIR/mtp_config"] && [! -f "$WORKDIR/mtproto-proxy" ];then
    echo "MTProxyTLS one-click installation and running green script"
    print_line
     install
     config_mtp
     run_mtp
     ...
     info_mtp
     print_line
     echo -e "Script source code: https://github.com/ellermister/mtproxy"
     echo -e "Configuration file: $WORKDIR/mtp_config"
     echo -e "Uninstall method: directly delete the files in the current directory"
     echo "How to use:"
     echo -e "\tStart the service bash $0 start"
     echo -e "\tDebug and run bash $0 debug"
     echo -e "\tStop the service bash $0 stop"
     echo -e "\tRestart the service bash $0 restart"
     echo -e "\tFix common problems bash $0 fix"
   fi
fi 
